...
namespace Net21WebStoreMVCProject.Models
{
    public class ShoppingCart
    {
        ...
        {
            ISession session = service.GetRequiredService<IHttpContextAccessor>()?.HttpContext.Session; // Provides session in web browser

            var context = service.GetService<AppDbContext>();
            string cartId = session.GetString("CartId") ?? Guid.NewGuid().ToString(); // Generates id for every cart
